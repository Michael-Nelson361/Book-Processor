...
class TextBlock:
    ...
    @property
    def has_significant_whitespace_above(self) -> bool:
        """Check if there's significant whitespace above this text block"""
        # This is a simplified check - in practice, you'd compare with other blocks on the page
        return self.bbox[1] > 100  # More than 100 points from top of page
    
    @property
    def is_at_page_top(self) -> bool:
        """Check if this text block is near the top of the page"""
        return self.bbox[1] < 150  # Within 150 points of page top
...
class ChapterDetector:
    ...
    def __init__(self, patterns: Optional[List[str]] = None, verbose: bool = True):
        ...
        # More restrictive chapter patterns - require specific chapter keywords
        ...
        # REMOVED: The problematic generic numeric pattern r'^\s*(\d+)\s*[:.]?\s*(.*)$'
        # This was causing false positives with exercise numbers, step numbers, etc.
        
        # More restrictive title-only patterns for books without explicit chapter markers
        self.title_patterns = [
            r'^([A-Z][A-Z\s]{8,60})$',  # ALL CAPS TITLE (8-60 chars, longer minimum)
            r'^(\d+)\s*[-–—]\s*([A-Z][A-Za-z\s]{5,50})$',  # 1 - Title (with proper title case)
        ]
        ...
        # TOC detection patterns - restrict to beginning of document
        self.toc_patterns = [
            re.compile(r'table\s+of\s+contents', re.IGNORECASE),
            re.compile(r'^\s*contents\s*$', re.IGNORECASE | re.MULTILINE),
        ]
        
        # Index patterns for end of document
        self.index_patterns = [
            re.compile(r'^\s*index\s*$', re.IGNORECASE | re.MULTILINE),
            re.compile(r'^\s*appendix\s*$', re.IGNORECASE | re.MULTILINE),
        ]
    ...
    def is_toc_page(self, text: str, page_num: int, total_pages: int) -> bool:
        """Check if the page is likely a table of contents - only in first 20% of document"""
        # Only check for TOC in the first 20% of the document
    ...
    def is_index_page(self, text: str, page_num: int, total_pages: int) -> bool:
        """Check if the page is likely an index - only in last 20% of document"""
        # Only check for index in the last 20% of the document
    ...
    def validate_chapter_context(self, text_block: TextBlock, doc_stats: DocumentStats, 
                                page_blocks: List[TextBlock]) -> Tuple[bool, float, List[str]]:
        """
        Validate if a text block is truly a chapter heading based on context
        Returns: (is_valid, confidence, reasons)
        """
        confidence = 0.0
        reasons = []
        
        # Factor 1: Font size significantly larger than body text
        size_ratio = text_block.font_size / doc_stats.body_font_size if doc_stats.body_font_size > 0 else 1.0
        if size_ratio >= 1.8:  # Much larger font
            confidence += 0.3
            reasons.append(f"large_font({size_ratio:.1f}x)")
        elif size_ratio >= 1.4:  # Moderately larger font
            confidence += 0.2
            reasons.append(f"medium_font({size_ratio:.1f}x)")
        
        # Factor 2: Bold formatting
        if text_block.is_bold:
            confidence += 0.25
            reasons.append("bold")
        
        # Factor 3: Centered on page
        if text_block.is_centered:
            confidence += 0.2
            reasons.append("centered")
        
        # Factor 4: At top of page or significant whitespace above
        if text_block.is_at_page_top:
            confidence += 0.15
            reasons.append("page_top")
        elif text_block.has_significant_whitespace_above:
            confidence += 0.1
            reasons.append("whitespace_above")
        
        # Factor 5: Different font family from body text
        if text_block.font_name != doc_stats.body_font_name:
            confidence += 0.1
            reasons.append("different_font")
        
        # Factor 6: Standalone on page (few other text blocks)
        if len(page_blocks) <= 3:  # Very few text blocks on page
            confidence += 0.1
            reasons.append("standalone")
        
        # Factor 7: Reasonable length for a chapter title
        text_len = len(text_block.text.strip())
        if 3 <= text_len <= 80:  # Reasonable title length
            confidence += 0.05
            reasons.append("good_length")
        
        # Penalty factors
        # Penalty 1: Too short or too long
        if text_len < 3 or text_len > 100:
            confidence -= 0.3
            reasons.append("bad_length")
        
        # Penalty 2: Contains common non-title indicators (expanded for textbooks)
        text_lower = text_block.text.lower()
        non_title_indicators = [
            'exercise', 'problem', 'figure', 'table', 'page', 'quick check', 
            'definition', 'procedure', 'theorem', 'lemma', 'proof', 'example',
            'solution', 'answer', 'note', 'remark', 'caution', 'warning',
            'summary', 'review', 'practice', 'homework', 'assignment'
        ]
        if any(indicator in text_lower for indicator in non_title_indicators):
            confidence -= 0.5
            reasons.append("non_title_indicator")
        
        # Penalty 3: Section numbers (like 1.1, 2.3, etc.) - these are subsections, not chapters
        if re.match(r'^\d+\.\d+', text_block.text.strip()):
            confidence -= 0.6
            reasons.append("section_number")
        
        # Penalty 4: Single words that are likely textbook elements
        single_word_non_chapters = [
            'calculus', 'algebra', 'geometry', 'trigonometry', 'preparedness',
            'instruction', 'supporting', 'transcendentals', 'forms', 'graphs',
            'elementary', 'functions', 'fundamental', 'integrals'
        ]
        if len(text_block.text.split()) == 1 and text_lower in single_word_non_chapters:
            confidence -= 0.4
            reasons.append("single_word_non_chapter")
        
        # Penalty 5: Starts with lowercase (unless it's a continuation)
        if text_block.text and text_block.text[0].islower():
            confidence -= 0.2
            reasons.append("lowercase_start")
        
        # Require multiple positive factors
        positive_factors = len([r for r in reasons if not r.startswith('bad_') and not r.startswith('non_') and not r.startswith('lowercase_')])
        if positive_factors < 2:
            confidence *= 0.5  # Reduce confidence significantly
            reasons.append("few_factors")
        
        # Cap confidence
        confidence = max(0.0, min(1.0, confidence))
        
        return confidence >= 0.6, confidence, reasons
    
    def detect_chapter_with_validation(self, text: str, page_num: int, text_blocks: List[TextBlock], 
                                     doc_stats: DocumentStats, total_pages: int) -> Optional[Tuple[str, str, str, float]]:
        """
        Detect if text contains a chapter heading with contextual validation
        Returns: (chapter_number, title, type, confidence) or None
        """
        # First, try pattern matching
        pattern_match = self.detect_chapter(text, page_num)
        if pattern_match:
            num, title, ch_type = pattern_match
            
            # Find the text block that contains this text
            matching_block = None
            for block in text_blocks:
                if text.strip() in block.text or block.text in text.strip():
                    matching_block = block
                    break
            
            if matching_block:
                # Validate the context
                is_valid, confidence, reasons = self.validate_chapter_context(
                    matching_block, doc_stats, text_blocks
                )
                
                if is_valid:
                    logger.debug(f"  ✓ Validated chapter on page {page_num + 1}: {reasons}")
                    return num, title, ch_type, confidence
                else:
                    logger.debug(f"  ✗ Rejected potential chapter on page {page_num + 1}: {reasons} (conf: {confidence:.2f})")
                    return None
            else:
                # No matching text block found - use pattern match with reduced confidence
                return num, title, ch_type, 0.8
        
        return None
    
    def detect_chapter(self, text: str, page_num: int = None) -> Optional[Tuple[str, str, str]]:
        """
        Detect if text contains a chapter heading using pattern matching only
...
class PDFChapterExtractor:
    ...
    def __init__(self, 
        ...
        # Set confidence thresholds based on sensitivity - more conservative for textbooks
        self.confidence_thresholds = {
            "low": 0.9,   # Very conservative
            "medium": 0.8, # Conservative (increased from 0.7)
            "high": 0.7   # Still higher than before (was 0.6)
        }
        self.confidence_threshold = self.confidence_thresholds.get(format_sensitivity, 0.8)
    ...
    def detect_chapters_with_format(self, doc: fitz.Document, doc_stats: DocumentStats) -> List[ChapterInfo]:
        ...
        for page_num in range(len(doc)):
            ...
            # Skip title pages (first few pages) for format detection to avoid false positives
            if page_num < 10:  # Skip first 10 pages which are often title, copyright, TOC, etc.
                continue
            ...
            try:
                ...
                for block in blocks:
                    ...
                    if is_heading and confidence > best_confidence:
                        # Additional filtering for textbook-specific false positives
                        text_lower = block.text.lower().strip()
                        
                        # Skip common textbook elements that aren't chapters
                        textbook_elements = [
                            'quick check answers', 'definition', 'definitions', 'procedure', 
                            'theorem', 'lemma', 'proof', 'example', 'solution', 'answer',
                            'note', 'remark', 'caution', 'warning', 'summary', 'review',
                            'alternative definition', 'forms of', 'graphs of'
                        ]
                        
                        # Skip section numbers (like 1.1, 2.3, etc.)
                        if re.match(r'^\d+\.\d+', block.text.strip()):
                            logger.debug(f"  ⚠️ Skipping section number on page {page_num + 1}: '{block.text}'")
                            continue
                        
                        # Skip single-word textbook elements
                        if any(element in text_lower for element in textbook_elements):
                            logger.debug(f"  ⚠️ Skipping textbook element on page {page_num + 1}: '{block.text}'")
                            continue
                        
                        # Skip very short or very long text
                        if len(block.text.strip()) < 4 or len(block.text.strip()) > 80:
                            continue
    ...
    def consolidate_duplicate_chapters(self, chapters: List[ChapterInfo]) -> List[ChapterInfo]:
        """Consolidate chapters with identical titles on consecutive pages"""
        if not chapters:
            return chapters
        ...
        for chapter in sorted(chapters, key=lambda x: x.start_page):
            if current_chapter is None:
                current_chapter = chapter
            elif (current_chapter.title.strip().lower() == chapter.title.strip().lower() and
                  current_chapter.end_page is not None and  # Safety check
                  chapter.start_page <= current_chapter.end_page + 3):  # Within 3 pages
                # Extend the current chapter
                if chapter.end_page:
                    current_chapter.end_page = max(current_chapter.end_page or current_chapter.start_page, chapter.end_page)
                logger.debug(f"  🔀 Consolidated '{chapter.title}' pages {current_chapter.start_page + 1}-{current_chapter.end_page + 1}")
            else:
                # Different chapter, add the previous one and start new
                consolidated.append(current_chapter)
                current_chapter = chapter
    ...
    def post_process_chapters(self, chapters: List[ChapterInfo], total_pages: int) -> List[ChapterInfo]:
        ...
        FIXES APPLIED:
        1. Calculate end_page BEFORE consolidation (fixes TypeError)
        2. Better filtering of textbook elements
        3. More conservative confidence requirements
        4. Improved validation and logging
        """
        if not chapters:
            return chapters
        ...
        # FIRST: Calculate end pages (BEFORE consolidation that depends on them)
        for i in range(len(chapters) - 1):
            chapters[i].end_page = chapters[i + 1].start_page - 1
        if chapters:
            chapters[-1].end_page = total_pages - 1
        
        # SECOND: Now consolidate duplicate titles (after end pages are calculated)
        chapters = self.consolidate_duplicate_chapters(chapters)
        ...
        for i, chapter in enumerate(chapters):
            ...
            # Keep pattern-detected chapters with lower page requirement (they're more reliable)
            if chapter.detection_method == "pattern":
                if page_count >= 1:  # Keep all pattern-detected chapters
                    filtered_chapters.append(chapter)
                else:
                    merged_count += 1
                    logger.debug(f"  🔀 Merging invalid pattern chapter: '{chapter.title}' ({page_count} pages)")
            # For format-detected chapters, use stricter requirements
            elif page_count >= self.min_chapter_pages and chapter.confidence >= 0.8:
                filtered_chapters.append(chapter)
            # Reject format detections that are short or low confidence
            else:
                merged_count += 1
                logger.debug(f"  🔀 Rejecting format chapter: '{chapter.title}' ({page_count} pages, conf: {chapter.confidence:.2f})")
        ...
        # Re-sort and recalculate end pages after filtering
        ...
        pattern_count = len([c for c in final_chapters if c.detection_method == "pattern"])
        format_count = len([c for c in final_chapters if c.detection_method == "format"])
        logger.debug(f"Final result: {pattern_count} pattern-detected + {format_count} format-detected = {len(final_chapters)} total chapters")
    ...
    def analyze_pdf(self, filepath: str) -> Tuple[List[ChapterInfo], fitz.Document]:
        ...
        # Standard pattern-based detection with enhanced validation
        for page_num in range(total_pages):
            ...
            try:
                ...
                # Check if this is a TOC page (only in first 20% of document)
                if self.detector.is_toc_page(text, page_num, total_pages):
                    logger.debug(f"  📑 Table of Contents detected at page {page_num + 1}")
                    toc_end_page = page_num + self.skip_toc_pages
                    continue
                
                # Check if this is an index page (only in last 20% of document)
                if self.detector.is_index_page(text, page_num, total_pages):
                    logger.debug(f"  📇 Index detected at page {page_num + 1}")
                    continue
                ...
                # Get text blocks for validation if doc_stats available
                text_blocks = []
                if doc_stats:
                    text_blocks = self.detector.extract_text_blocks(page, page_num)
                
                # Check for chapters/sections using patterns with validation
                lines = text.strip().split('\n')[:10]  # Check first 10 lines
                
                for line in lines:
                    if doc_stats and text_blocks:
                        # Use enhanced validation
                        result = self.detector.detect_chapter_with_validation(
                            line, page_num, text_blocks, doc_stats, total_pages
                        )
                        if result:
                            num_val, title_val, ch_type, confidence = result
                        else:
                            continue
                    else:
                        # Fallback to basic pattern matching
                        basic_result = self.detector.detect_chapter(line, page_num)
                        if basic_result:
                            num_val, title_val, ch_type = basic_result
                            confidence = 1.0
                        else:
                            continue
                    ...
                    if not any(c.start_page == page_num for c in chapters):
                        ...
                        logger.debug(f"  📖 {type_str} {num_display} found: '{chapter_info.title}' at page {page_num + 1} (conf: {confidence:.2f})")
                    break
            except Exception as e:
    ...
    def extract_chapters(self, filepath: str, save_metadata: bool = True) -> Dict[str, any]:
        ...
        for i, chapter in enumerate(chapters):
            if chapter.end_page is None:
                logger.error(f"Chapter {i+1} '{chapter.title}' has no end_page - this should not happen after post-processing!")
                continue
...
Examples:
  ...
  %(prog)s book.pdf --log-file extract.log # Save detailed log
    ...
    if args.log_file:
        ...
        # Generate log filename based on input file with better naming
        book_path = Path(args.filepath)
        book_name = book_path.stem
        # Create timestamp
        from datetime import datetime
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        # Create descriptive log filename: BookTitle_ChapterExtractorLog_YYYYMMDD_HHMMSS.txt
        log_file = book_path.parent / f"{book_name}_ChapterExtractorLog_{timestamp}.txt"
...
if __name__ == "__main__":
    ...
    def validate_chapter_context(self, text_block: TextBlock, doc_stats: DocumentStats, 
        ...
        # Penalty 2: Contains common non-title indicators
        text_lower = text_block.text.lower()
        if any(indicator in text_lower for indicator in ['exercise', 'problem', 'figure', 'table', 'page']):
            confidence -= 0.4
            reasons.append("non_title_indicator")
        
        # Penalty 3: Starts with lowercase (unless it's a continuation)
        if text_block.text and text_block.text[0].islower():
            confidence -= 0.2
            reasons.append("lowercase_start")
...
class PDFChapterExtractor:
    ...
    def post_process_chapters(self, chapters: List[ChapterInfo], total_pages: int) -> List[ChapterInfo]:
        ...
        # First, consolidate duplicate titles
        chapters = self.consolidate_duplicate_chapters(chapters)
...
if __name__ == "__main__":
    main(),  # IRON RULE I
            # Pattern for common literary sections
